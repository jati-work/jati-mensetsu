...
const VocabHub: React.FC<Props> = ({ vocabList, setVocabList }) => {
    ...
    const exportData = () => {
        ...
        a.href = url;
        a.download = `vocab_jati_${new Date().toISOString().split('T')[0]}.json`;
        a.click();
    };
        ...
        <div className="space-y-12 fade-in pb-20 pt-4 md:pt-0">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div className="space-y-2">
                    ...
                    <p className="text-indigo-500 font-black text-[10px] uppercase tracking-[0.3em]">Tekan & Geser kartu untuk ganti urutan</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={exportData} className="p-4 bg-gray-900 text-white rounded-2xl shadow-lg hover:bg-black transition-all" title="Export JSON"><Download size={20}/></button>
                    <label className="p-4 bg-white border border-gray-100 text-gray-400 rounded-2xl shadow-sm cursor-pointer hover:bg-gray-50 transition-all" title="Import JSON">
                        <Upload size={20}/>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scroll space-y-3 pr-2">
                        {filteredList.map((item) => (
                            <div 
                                ...
                                className={`p-5 rounded-3xl border flex items-center justify-between transition-all cursor-grab active:cursor-grabbing hover:shadow-lg group ${draggedId === item.id ? 'opacity-20 scale-95 border-indigo-300' : 'bg-white border-gray-100'}`}
                            >
                                <div className="flex items-center gap-4">
                                    <GripVertical size={18} className="text-gray-200 group-hover:text-indigo-200 transition-colors" />
                                    <div>
