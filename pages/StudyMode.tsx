                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 slide-up">
                    <div className="bg-[#f0c3e6] p-8 rounded-[48px] shadow-lg border-4 border-white space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={() => setInterviewPoints([...interviewPoints, ""])} className="p-2 bg-white/40 rounded-xl"><PlusCircle size={20}/></button>
                        </div>
                        <div className="space-y-3">
                            {interviewPoints.map((point, i) => (
                                <div key={i} className="flex gap-3 group">
                                    <input value={point} onChange={(e) => setInterviewPoints(interviewPoints.map((p, idx) => idx === i ? e.target.value : p))} placeholder="Poin penting..." className="flex-1 text-sm font-black text-gray-900 bg-white/20 p-4 rounded-xl border border-white/30 outline-none" />
                                    <button onClick={() => setInterviewPoints(interviewPoints.filter((_, idx) => idx !== i))} className="text-rose-600 opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-[#f0c3e6] p-8 rounded-[48px] shadow-lg border-4 border-white space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={() => setEmergencyPhrases([...emergencyPhrases, ""])} className="p-2 bg-white/40 rounded-xl"><PlusCircle size={20}/></button>
                        </div>
                        <div className="space-y-3">
                            {emergencyPhrases.map((phrase, i) => (
                                <div key={i} className="flex gap-3 group">
                                    <textarea value={phrase} onChange={(e) => setEmergencyPhrases(emergencyPhrases.map((p, idx) => idx === i ? e.target.value : p))} placeholder="Frasa penyelamat..." className="flex-1 text-sm font-black text-gray-900 bg-white/20 p-4 rounded-xl border border-white/30 outline-none resize-none h-20" />
                                    <button onClick={() => setEmergencyPhrases(emergencyPhrases.filter((_, idx) => idx !== i))} className="text-rose-600 opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-[#f0c3e6] p-8 rounded-[48px] shadow-lg border-4 border-white space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={() => setStudyNotes([...studyNotes, ""])} className="p-2 bg-white/40 rounded-xl"><PlusCircle size={20}/></button>
                        </div>
                        <div className="space-y-3">
                            {studyNotes.map((note, i) => (
                                <div key={i} className="flex gap-3 group">
                                    <input value={note} onChange={(e) => setStudyNotes(studyNotes.map((n, idx) => idx === i ? e.target.value : n))} placeholder="Catatan belajar..." className="flex-1 text-sm font-black text-gray-900 bg-white/20 p-4 rounded-xl border border-white/30 outline-none" />
                                    <button onClick={() => setStudyNotes(studyNotes.filter((_, idx) => idx !== i))} className="text-rose-600 opacity-0 group-hover:opacity-100"><Trash2 size={18} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
                                ...
                                {showAnswer && (
                                    <div className="bg-indigo-50 p-10 rounded-[48px] border border-indigo-100 space-y-4 slide-up">
                                        <div className="space-y-1">
                                            <p className="text-3xl font-black text-indigo-900">{currentQ.answerJapanese}</p>
                                            <p className="text-xs font-bold text-indigo-400 italic">"{currentQ.answerRomaji}"</p>
                                        </div>
                                        <p className="text-[11px] font-bold text-gray-400 italic mt-4 border-t border-indigo-100/50 pt-4">"{currentQ.answerIndo}"</p>
                                    </div>
                                )}
